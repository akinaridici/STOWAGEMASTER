"""Genetic Algorithm optimizer with progress reporting for tanker stowage planning"""
...
class GeneticOptimizerWithProgress:
    """Genetic Algorithm optimizer with progress reporting for tanker stowage planning"""
    ...
    def __init__(self, ship: Ship, cargo_requests: List[Cargo], 
        ...
        """Initialize genetic optimizer with progress reporting
    ...
    def _place_mandatory_cargos(self, available_tanks: Dict[str, float], 
        ...
        self._report_progress(
            ...
            0.0, 
            f"Placing {len(self.mandatory_cargos)} mandatory cargos"
        )
        ...
        for i, cargo in enumerate(self.mandatory_cargos):
            if self._check_cancellation():
                break
                
            # Report progress for mandatory cargo placement
            progress = (i + 1) / len(self.mandatory_cargos) * 100
            self._report_progress(
                ProgressStage.PLACING_MANDATORY,
                progress,
                f"Placing mandatory cargo {i+1}/{len(self.mandatory_cargos)}: {cargo.name}"
            )
    ...
    def create_initial_population(self) -> List[Chromosome]:
        ...
        self._report_progress(
            ...
            0.0,
            f"Creating initial population of {self.population_size} chromosomes"
        )
        ...
        for i in range(self.population_size):
            ...
            # Report progress for population creation
            progress = (i + 1) / self.population_size * 100
            self._report_progress(
                ProgressStage.INITIALIZING_POPULATION,
                progress,
                f"Creating chromosome {i+1}/{self.population_size}"
            )
    ...
    def optimize(self) -> StowagePlan:
        """Run genetic algorithm optimization with progress reporting
        ...
        if self._check_cancellation():
            return self._create_cancelled_plan()
        ...
        # Report initial best fitness
        self._report_progress(
            ProgressStage.RUNNING_OPTIMIZATION,
            0.0,
            f"Starting optimization with {self.max_generations} generations, best fitness: {best_fitness:.2f}"
        )
        ...
        for generation in range(self.max_generations):
            ...
            # Report progress for generation
            progress = (generation + 1) / self.max_generations * 100
            self._report_progress(
                ProgressStage.RUNNING_OPTIMIZATION,
                progress,
                f"Generation {generation+1}/{self.max_generations}, best fitness: {best_fitness:.2f}"
            )
            ...
            if len(self.best_fitness_history) >= self.convergence_generations:
                ...
                if recent_improvement < self.convergence_threshold:
                    ...
                    self._report_progress(
                        ProgressStage.RUNNING_OPTIMIZATION,
                        100.0,
                        f"Optimization converged after {generation+1} generations, best fitness: {best_fitness:.2f}"
                    )
                    break
        ...
        self._report_progress(
            ...
            0.0,
            "Converting best solution to stowage plan"
        )
        ...
        self._report_progress(
            ...
            0.0,
            "Post-processing: filling empty tanks with remaining cargo"
        )
    ...
    def _create_cancelled_plan(self) -> StowagePlan:
